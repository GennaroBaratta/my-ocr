...
class ReviewScreenActions:
    ...
    def prev_page(self, _e: object | None = None) -> None:
        if not self.state.select_previous_page():
            return
        self._page_changed()

    def next_page(self, _e: object | None = None) -> None:
        if not self.state.select_next_page():
            return
        self._page_changed()

    def select_page(self, idx: int) -> None:
        self.state.set_current_page_index(idx)
        self._page_changed()
    ...
    def toggle_add_box(self, _e: object | None = None) -> None:
        self.state.set_add_box_mode(not self.state.session.is_adding_box)
