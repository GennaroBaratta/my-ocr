...
def extract_from_ocr_payload(payload: dict[str, Any]) -> dict[str, Any]:
    markdown = payload.get("markdown") or payload.get("ocr_markdown") or ""
    if not markdown and isinstance(payload.get("json"), dict):
        markdown = _markdown_from_json(payload["json"])
...
def _markdown_from_json(payload: dict[str, Any]) -> str:
