...
from .page_processor import GlmOcrPageProcessor
...
def run_ocr(
    ...
    *,
    ...
    try:
        for page_ref in page_inputs:
            ...
            if review_page is not None:
                page_result = page_processor.process_review_layout_page(
                    page_path,
                    page_number,
                ...
                if parser is None:
                    next_parser = parser_cls(
                        config_path=options.config_path,
                        layout_device=options.layout_device,
                        _dotted=layout_dotted,
                    )
                    next_parser.__enter__()
                    parser = next_parser
                page_result, parser_retired = page_processor.process_sdk_page(
                    parser,
                    page_path,
                    page_number,
    ...
    markdown = "\n\n".join(
        page["markdown"] for page in page_payloads if page["markdown"].strip()
    )
